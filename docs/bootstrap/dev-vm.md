...
We're going to build a node to host our WP stack. This base machine
packs a fully-fledged, one-node WP cluster—yea, that's a bit of an
...
- All the services you'd expect in a full-blown WP installation.
  Namely: Nginx, Wordpress, redis, Mariadb, php-fpm, etc.
- Firewall. You can easily turn it off if it really comes in your
  ...
  prod clone.
- Root access (DO NOT USE THIS IN PRODUCTION DUDE!!)
- Remote access through SSH.
- Basic Linux sys admin tools like `curl`, `ncdu`, `zip / unzip`, etc. You
  can find the full list in each node's configuration.nix file, under the `environment.systemPackages` directive. 


### Hardware

Ideally, you have a box with at least 4 CPUs, 8GB of RAM and 50GB
SSD storage. But you can get away with just 2 CPUs, 4GB RAM and 30GB
...
After provisioning the hardware, you install our NixOS/WP stack.
...
Download the NixOS 25.05 image and boot it on the designated victim,
...
the sake of having a concrete example, we use Qemu -- if you're already running the nix pkg manager on your local machine, it would be a no-brainer to get it. Check [Nix Search](https://search.nixos.org/packages?channel=unstable&query=qemu) for more info.
...
```bash
# the commands below will only work **after** you entered a nix shell with qemu `nix-shell -p qemu` or got it using `nix-env -iA nixpkgs`
...
port_fwd+='hostfwd=tcp::3306-:3306'
...
With a freshly minted NixOS VM in our hands, installing our WP
stack is a walk in the park. All you need to do is deploy the `devm` NixOS
